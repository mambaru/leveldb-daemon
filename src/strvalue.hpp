...
template<>
struct strvalue<std::string>
{
  typedef std::string target;
  typedef serializerT< strvalue<std::string> > serializer;
};

template<>
class serializerT< strvalue<std::string> > {
public:
	...
	P operator() ( std::string& v, P beg, P end ) {
		v.clear();
		if ( parser::is_null(beg, end) ) return parser::parse_null(beg, end);
	...
	P unserialize(P beg, P end, P1 vitr, int n = -1)
	{
		...
		if (require_quote) {
			...
			for ( ;beg!=end && *beg!='"' && n!=0; )
			{
				...
				} else beg = _unserialize_symbol(beg, end, &vitr, n);
			};
			...
			for ( ;beg!=end && *beg!='}' && *beg!=' ' && *beg!=']' && *beg!=',' && n!=0; ) {
