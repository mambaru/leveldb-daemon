#ifndef FAS_ADV_JSON_RPC2_AD_INVOKE_HPP
#define FAS_ADV_JSON_RPC2_AD_INVOKE_HPP
...
// TODO: сделать аттрибутом
    ...
    if ( method==0 && result==0) { t.get_aspect().template get<_invalid_json_>()(t, d, s ); return; }
    if ( result!=0 && id==0) { t.get_aspect().template get<_invalid_json_>()(t, d, s ); return; }
    ...
    if ( method!=0 && id!=0 && result==0)
    {
      ...
      _invoke(t, invoke_list(), method, d, s, -1);  // request
    }
    else if ( method!=0 && id==0 && result==0)
      _invoke(t, invoke_list(), method, d, s, -2);  // notify
    else if (result!=0 && id!=0 && method==0)
      _invoke(t, invoke_list(), result, d, s, rid); // response
    else
    {
      t.get_aspect().template get<_invalid_json_>()(t, d, s ); return; 
    }
    */
  }
  ...
  const char* _invoke_1 (T& t, const char* d, size_t s)
  {
    ...
    if ( method==0 && result==0) { t.get_aspect().template get<_invalid_json_>()(t, d, s ); return next; }
    if ( result!=0 && id==0) { t.get_aspect().template get<_invalid_json_>()(t, d, s ); return next; }
...
private:
  ...
  const char *_search_tovalue_( const char *beg, const char *end)
  {
    ...
    if ( /*beg==end ||*/ *beg!=':') 
      beg = end; 
    /*{
      t.get_aspect().template get<_invalid_json_>()(t, beg, end-beg );
      return end;
    }*/
    if (beg!=end)
      ++beg;
    ...
    if ( beg==end ) 
      throw fas::misc::fast_json::invalid_json();
      /*t.get_aspect().template get<_invalid_json_>()(t, start, end-start );*/
    return beg;
  }
  ...
  const char* _invoke(T& t, L, const char* name, const char* d, typename T::size_type s, int code)
  {
    const char* next = 0;
    typedef typename T::aspect::advice_list advice_list;
    typedef typename ap::select<_gmethod_, advice_list>::type invoke_list;
  ...
  template< typename T>
  const char* _invoke(T& t, ap::empty_type, const char* name, const char* d, typename T::size_type s, int code)
  {
    if ( code < 0 )
